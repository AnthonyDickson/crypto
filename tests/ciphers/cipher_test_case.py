...
from crypto.types import Message, Key, KeySpace, CipherText
...
class CipherTestCase(unittest.TestCase):
    ...
    def keyspace_is_correct_test(self, cipher_type: Type[CipherABC], expected_key_space: Union[KeySpace, Callable]):
        ...
        self.assertSetEqual(expected_key_space if type(expected_key_space) is set else set(expected_key_space()),
                            set(cipher_type().key_space()),
                            msg='The key space for CaesarCipher is incorrect.')

    def can_set_key_test(self, cipher_type: Type[CipherABC], raw_key: Any, expected_key: Key):
    ...
    def generates_valid_ciphertext_test(self, cipher_type: Type[CipherABC], key: Optional[Key] = None):
    ...
    def generates_valid_plaintext_test(self, cipher_type: Type[CipherABC], key: Optional[Key] = None):
    ...
    def is_symmetric_test(self, cipher_type: Type[CipherABC], key: Optional[Key] = None):
